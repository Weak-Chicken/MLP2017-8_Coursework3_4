...
output = tf.layers.dense(outputs[:, -1, :], num_classes, use_bias=True,
                         bias_initializer=tf.constant_initializer(0.1, dtype=tf.float32),
                         kernel_initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32)) # output based on the last output step
...
for i in range(iteration_times):
    ...
    # temp_o = sess.run(output,{x:local_x, y:local_y, keep_prob: 1.0, batch_size: _batch_size})
