...
# DEFAULT_DATA_PATH = 'data/' # For old data inside codes
DEFAULT_DATA_PATH = 'D:/MLP_DATA/'
...
class MFCCDataProvider(DataProvider):
    """Data provider for  Free Music Archive (FMA) MFCC version."""

    def __init__(self, track_num, which_set='train', batch_size=100, max_num_batches=-1,
                 shuffle_order=True, rng=None, flatten=False, one_hot=False):
        """Create a new FMA-MFCC data provider object.
        ...
        # MLP_DATA_DIR environment variable should point to the data directory
        data_path = os.path.join(
            DEFAULT_DATA_PATH, 'MFCC-{0}_{1}.npz'.format(which_set, str(track_num)))
        assert os.path.isfile(data_path), (
                'Data file does not exist at expected path: ' + data_path
        )
        # load data from compressed numpy file
        ...
        inputs = inputs.astype(np.float32)
        if flatten:
            inputs = np.reshape(inputs, newshape=(-1, 36374))
        else:
            inputs = np.reshape(inputs, newshape=(-1, 13, 2798))
            # inputs = np.expand_dims(inputs, axis=3)

        # pass the loaded data to the parent class __init__
        super(MFCCDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)

    def next(self):
        """Returns next data batch or raises `StopIteration` if at end."""
        inputs_batch, targets_batch = super(MFCCDataProvider, self).next()
        if self.one_hot:
            return inputs_batch, self.to_one_of_k(targets_batch)
        else:
            return inputs_batch, targets_batch
